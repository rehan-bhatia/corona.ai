...
class SignInCard extends React.Component {
	...
	SignIn = () => {
	    ...
	    Auth.signIn({username: email, password: pass})
	    	.then((response)=>{
	    		navigate("/dashboard", {state:{clientId: response.pool.clientId, email: this.state.email}})
	...
	render() {
		if (this.state.remember && (typeof window !== 'undefined')) {
		...
		return(
			<div className="w-100 shadow-3 pa2 pt0 pb4 tc bg-white ba b--light-gray Avenir ml4" style={{width: (isMobile) ? "90vw" : "650px"}}>
				...
				<p onClick={()=>OnSubmit()} class={`f4 tc no-underline black bg-animate hover-bg-purple hover-white inline-flex pointer mb3 items-center pa3 ba border-box ph4 br3 ${(isMobile) ? "mr3" : "mr5"}`} style={{"margin-left":"auto"}}>
				...
				<br/>
				<p class="f4 tc no-underline bg-animate bg-light-purple light-gray inline-flex pointer mb3 items-center grow mt4 bg-white black pa3 ph4 br3" style={{"margin-left":"auto"}}>
				    <span class="pr1"> Invite Others </span>
				</p>
			</div>
		);
	}
